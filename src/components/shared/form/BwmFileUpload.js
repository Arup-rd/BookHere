...
import firebase, { storage } from 'firebase-conf/config'
...
export class BwmFileUpload extends React.Component {
  ...
  uploadImage() {
    ...
    // let filename = `${Date.now()}-${uuidv1()}.${ext[1]}`;
    // console.log(croppedImage)

    // Create Storage Ref
    let storageRef = storage.ref('arup-rd/' + croppedImage.name);
    
    // Upload file
    let task = storageRef.put(croppedImage)
  ...
  render() {
    ...
    return (
      <div className='img-upload-container'>
        <label className='img-upload btn btn-bwm bg-info text-white'>
